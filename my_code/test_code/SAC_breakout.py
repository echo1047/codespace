...
class PolicyNet(torch.nn.Module):
    ...
    def __init__(self, state_in_channels, num_hidden_units, num_actions):
        super(PolicyNet, self).__init__()
        # self.fc1 = torch.nn.Linear(state_dim, hidden_dim) #todo 换成con2d
        # self.fc2 = torch.nn.Linear(hidden_dim, action_dim)
        self.fc1 = torch.nn.Conv2d(
            in_channels=state_in_channels, out_channels=num_hidden_units, kernel_size=3)
        self.fc2 = torch.nn.Linear(num_hidden_units, num_actions)
...
if __name__ == '__main__':
    ...
    env: gym.Env = gym.make(env_name, render_mode='rgb_array_list')
    state_in_channels: int = np.array(env.observation_space.shape).shape[0]
    num_actions = env.action_space.n # type: ignore
    replay_buffer = my_rl_utils.ReplayBuffer(buffer_size)

    agent = SAC(state_in_channels, num_hidden_units, num_actions, actor_lr, critic_lr, alpha_lr,
